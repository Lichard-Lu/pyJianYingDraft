# pyJianYingDraft
### 轻量、灵活、易上手的Python剪映/CapCut草稿生成及导出工具，构建全自动视频剪辑/混剪流水线！

> ✨ **新功能**：本项目现已支持 **CapCut**！通过 `app_type` 参数即可在剪映和CapCut之间切换。
...
#### Windows
- 支持剪映和CapCut的草稿生成、模板模式和自动导出功能
- 自动导出功能通过 `JianyingController` 实现（需要 uiautomation）
- 具体功能可能受到应用版本限制

#### macOS
- 支持剪映和CapCut的草稿生成和模板模式
- **CapCut 在 macOS 上需要 draft_info.json 文件**（自动生成，无需手动处理）
- 不支持自动导出（uiautomation 仅支持 Windows）
- 生成的草稿可以在 macOS 版应用中使用

#### Linux
- 支持草稿生成和模板模式
- 不支持自动导出
- 生成的草稿需要在 Windows 或 macOS 版应用中使用
...
使用CapCut时，只需在创建`DraftFolder`时指定`app_type="capcut"`：

```python
import pyJianYingDraft as draft

# 指定使用 CapCut
draft_folder = draft.DraftFolder("<CapCut草稿文件夹>", app_type="capcut")
script = draft_folder.create_draft("demo", 1920, 1080)

# 添加素材和片段...
# ...

script.save()  # macOS上会自动生成 draft_info.json
```

**注意**：
- macOS 用户：CapCut 在 macOS 上需要 `draft_info.json` 文件，保存时会自动生成
- Windows 用户：CapCut 在 Windows 上不需要 `draft_info.json` 文件
...
> ⚠️ 剪映7+版本对控件进行了隐藏，故本功能目前**仅支持剪映6及以下版本和CapCut**

> ⚠️ 本部分功能依赖于`uiautomation`库，故目前**仅支持在Windows系统下运行**

> ℹ 导出程序会将应用窗口置顶，且需要控制光标进行点击，**建议在闲时/夜间运行**
...
导出利用`JianyingController`类进行，该类现在同时支持剪映和CapCut。
...
# 此前需要将剪映打开，并位于目录页
ctrl = draft.JianyingController()  # 默认使用剪映
...
# 指定 app_type="capcut" 来使用 CapCut
ctrl = draft.JianyingController(app_type="capcut")
