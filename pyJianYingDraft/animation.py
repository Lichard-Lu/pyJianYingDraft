...
class VideoAnimation(Animation):
    ...
    def __init__(self, animation_type: Union[IntroType, OutroType, GroupAnimationType, CapCut_Intro_type, CapCut_Outro_type, CapCut_Group_animation_type],
                 start: int, duration: int):
        super().__init__(animation_type.value, start, duration)

        if isinstance(animation_type, (IntroType, CapCut_Intro_type)):
            self.animation_type = "in"
        elif isinstance(animation_type, (OutroType, CapCut_Outro_type)):
            self.animation_type = "out"
        elif isinstance(animation_type, (GroupAnimationType, CapCut_Group_animation_type)):
...
class Text_animation(Animation):
    ...
    def __init__(self, animation_type: Union[TextIntro, TextOutro, TextLoopAnim, CapCut_Text_intro, CapCut_Text_outro, CapCut_Text_loop_anim],
                 start: int, duration: int):
        super().__init__(animation_type.value, start, duration)

        if isinstance(animation_type, (TextIntro, CapCut_Text_intro)):
            self.animation_type = "in"
        elif isinstance(animation_type, (TextOutro, CapCut_Text_outro)):
            self.animation_type = "out"
        elif isinstance(animation_type, (TextLoopAnim, CapCut_Text_loop_anim)):
