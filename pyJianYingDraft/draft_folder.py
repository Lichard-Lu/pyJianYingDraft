...
class DraftFolder:
    ...
    def __init__(self, folder_path: str, app_type: str = "jianying"):
        """初始化草稿文件夹管理器

        Args:
            folder_path (`str`): 包含若干草稿的文件夹, 一般取剪映/CapCut保存草稿的位置即可
    ...
    def has_draft(self, draft_name: str) -> bool:
        ...
        注意: 本函数只检查文件夹是否存在, 并不检查草稿是否符合剪映/CapCut的格式
    ...
    def create_draft(self, draft_name: str, width: int, height: int, fps: int = 30, *,
        ...
        if self.app_type == "capcut":
            template_name = "DRAFT_META_TEMPLATE_CAPCUT"
        else:
            template_name = "DRAFT_META_TEMPLATE"
        shutil.copy(assets.get_asset_path(template_name), os.path.join(draft_path, "draft_meta_info.json"))

        # 创建草稿文件，传递 app_type 参数
        script_file = ScriptFile(width, height, fps, maintrack_adsorb, app_type=self.app_type)
