...
class ScriptFile:
    ...
    def __init__(self, width: int, height: int, fps: int, maintrack_adsorb: bool, 
                 app_type: Literal["jianying", "capcut"] = "jianying"):
        """**创建草稿推荐使用`DraftFolder.create_draft()`而非此方法**
    ...
    def dumps(self) -> str:
        ...
        # 根据 app_type 更新 last_modified_platform 和 platform 字段
        # CapCut 使用 "cc", 剪映使用 "lv"
        if self.app_type == "capcut":
            capcut_platform = {
                "app_id": 359289,
                "app_source": "cc",
                "app_version": "6.7.0",
            ...
            self.content["last_modified_platform"] = capcut_platform
            if "platform" in self.content:
                self.content["platform"] = capcut_platform.copy()
        else:
            # 剪映使用默认的 lv 设置
            jianying_platform = {
                "app_id": 3704,
            ...
            self.content["last_modified_platform"] = jianying_platform
            if "platform" in self.content:
                self.content["platform"] = jianying_platform.copy()
    ...
    def save(self) -> None:
        ...
        # 跨平台兼容性处理
        # macOS CapCut 需要 draft_info.json 文件
        ...
        # 仅在 macOS 上且 app_type 为 capcut 时创建 draft_info.json
        if is_macos and self.app_type == "capcut":
            draft_info_path = os.path.join(draft_dir, "draft_info.json")
            
            # 获取草稿名称（从文件夹名称）
            draft_name = os.path.basename(draft_dir)
            
            # 读取刚保存的 draft_content.json 内容
            with open(self.save_path, "r", encoding="utf-8") as f:
                draft_info_content = json.load(f)
            
            # 添加 draft_info.json 需要的额外字段
            draft_info_content["draft_type"] = "video"
            draft_info_content["name"] = ""  # CapCut 期望空字符串，名称从 draft_meta_info.json 读取
            draft_info_content["path"] = ""
            draft_info_content["new_version"] = "149.0.0"
            draft_info_content["version"] = 360000
            draft_info_content["update_time"] = int(time.time() * 1000000)  # 微秒时间戳
            draft_info_content["source"] = "default"
            draft_info_content["static_cover_image_path"] = ""
            draft_info_content["time_marks"] = None
            draft_info_content["mutable_config"] = None
            draft_info_content["retouch_cover"] = None
            draft_info_content["relationships"] = []
            draft_info_content["render_index_track_mode_on"] = True
            draft_info_content["smart_ads_info"] = {
                "draft_url": "",
                "page_from": "",
                "routine": ""
            }
            draft_info_content["uneven_animation_template_info"] = {
                "composition": "",
                "content": "",
                "order": "",
                "sub_template_info_list": []
            }
            draft_info_content["function_assistant_info"] = {
            ...
            # 更新 platform 信息（如果存在）
            if "last_modified_platform" in draft_info_content:
                draft_info_content["platform"] = draft_info_content["last_modified_platform"].copy()
                if "os" in draft_info_content["platform"]:
                    draft_info_content["platform"]["os"] = platform.system().lower()[:3]  # "mac" or "win"
            
            # 保存 draft_info.json
            with open(draft_info_path, "w", encoding="utf-8") as f:
                json.dump(draft_info_content, f, ensure_ascii=False, indent=4)
        
        # 更新 draft_meta_info.json 中的 draft_materials
        # macOS CapCut 需要此信息来链接媒体文件
        # Windows 也可能需要，所以统一处理（参考 pyCapCut 的实现）
        draft_dir = os.path.dirname(self.save_path)
        draft_meta_path = os.path.join(draft_dir, "draft_meta_info.json")
        if os.path.exists(draft_meta_path):
            # 仅在 macOS 上且 app_type 为 capcut 时，需要特殊处理（复制文件到 Resources 等）
            copy_to_resources = is_macos and self.app_type == "capcut"
            
            # 始终更新 tm_duration 字段（剪映和CapCut都需要）
            with open(draft_meta_path, "r", encoding="utf-8") as f:
                draft_meta = json.load(f)
            draft_meta["tm_duration"] = self.duration / 1000000.0  # 转换为秒
            with open(draft_meta_path, "w", encoding="utf-8") as f:
                json.dump(draft_meta, f, ensure_ascii=False, indent=4)
            
            if copy_to_resources or self.app_type == "capcut":
                # 重新读取（因为上面已经更新了tm_duration）
                with open(draft_meta_path, "r", encoding="utf-8") as f:
                    draft_meta = json.load(f)
                ...
                # 从 draft_content.json 中读取素材信息
                with open(self.save_path, "r", encoding="utf-8") as f:
                    content = json.load(f)
                    materials = content.get("materials", {})
                
                # 创建 Resources 文件夹用于存储媒体文件
                # macOS CapCut 需要将文件复制到 Resources 文件夹以避免权限问题
                ...
                for video_data in materials.get("videos", []):
                    if "path" in video_data:
                        original_path = video_data.get("path", "")
                        # 复制文件到 Resources 文件夹
                        if os.path.exists(original_path) and os.path.dirname(original_path) != resources_dir:
                            filename = os.path.basename(original_path)
                            dest_path = os.path.join(resources_dir, filename)
                            if not os.path.exists(dest_path):
                                shutil.copy2(original_path, dest_path)
                            file_path = dest_path
                            path_mapping[original_path] = file_path
                        else:
                        ...
                        video_materials.append({
                            ...
                            "item_source": 1,
                            "md5": "",
                            "metetype": video_data.get("type", "video"),
                            "roughcut_time_range": {
                                "duration": video_data.get("duration", 0),
                                "start": 0
                            },
                ...
                # 更新音频素材（type 1）
                audio_materials = []
                for audio_data in materials.get("audios", []):
                    if "path" in audio_data:
                        original_path = audio_data.get("path", "")
                        # 根据平台决定是否复制文件到 Resources 文件夹
                        if copy_to_resources and os.path.exists(original_path) and os.path.dirname(original_path) != resources_dir:
                            filename = os.path.basename(original_path)
                            dest_path = os.path.join(resources_dir, filename)
                            if not os.path.exists(dest_path):
                                shutil.copy2(original_path, dest_path)
                ...
                # 更新 draft_materials 数组
                draft_materials = draft_meta.get("draft_materials", [])
                for material_group in draft_materials:
                    if material_group.get("type") == 0:  # 视频
                        material_group["value"] = video_materials
                    elif material_group.get("type") == 1:  # 音频
                        material_group["value"] = audio_materials
                
                draft_meta["draft_materials"] = draft_materials
                
                # 更新草稿路径信息（macOS CapCut 需要这些字段，Windows 也可以设置）
                if is_macos:
                    draft_meta["draft_fold_path"] = draft_dir
                    # draft_root_path 应该是草稿文件夹的父目录
                    draft_root = os.path.dirname(draft_dir)
                    draft_meta["draft_root_path"] = draft_root
                
                # 保存更新后的 draft_meta_info.json（tm_duration已在上面更新）
                with open(draft_meta_path, "w", encoding="utf-8") as f:
                    json.dump(draft_meta, f, ensure_ascii=False, indent=4)
                
                # 更新 draft_content.json 和 draft_info.json 中的路径
                if path_mapping:
                    ...
                    for video in content.get("materials", {}).get("videos", []):
                        if "path" in video and video["path"] in path_mapping:
                            video["path"] = path_mapping[video["path"]]
                    
                    # 更新音频路径
                    for audio in content.get("materials", {}).get("audios", []):
                        if "path" in audio and audio["path"] in path_mapping:
                            audio["path"] = path_mapping[audio["path"]]
                    
                    # 保存更新后的 draft_content.json
                    with open(self.save_path, "w", encoding="utf-8") as f:
                        json.dump(content, f, ensure_ascii=False, indent=4)
                    
                    # 更新 draft_info.json（macOS 需要，如果存在则更新）
                    if is_macos and draft_info_path and os.path.exists(draft_info_path):
                        with open(draft_info_path, "r", encoding="utf-8") as f:
                            info_content = json.load(f)
                        
                        # 更新视频路径
                        for video in info_content.get("materials", {}).get("videos", []):
                            if "path" in video and video["path"] in path_mapping:
                                video["path"] = path_mapping[video["path"]]
                        
                        # 更新音频路径
                        for audio in info_content.get("materials", {}).get("audios", []):
                            if "path" in audio and audio["path"] in path_mapping:
                                audio["path"] = path_mapping[audio["path"]]
