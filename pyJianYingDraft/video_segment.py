...
class VideoSegment(VisualSegment):
    ...
    def add_transition(self, transition_type: Union[TransitionType, CapCut_Transition_type], *, duration: Optional[Union[int, str]] = None) -> "VideoSegment":
        """为视频片段添加转场, 注意转场应当添加在**前面的**片段上

        Args:
            transition_type (`TransitionType` or `CapCut_Transition_type`): 转场类型
